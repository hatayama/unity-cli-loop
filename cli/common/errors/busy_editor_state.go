...
func unityServerBusyNextActions(data serverBusyErrorData) []string {
	...
	if optionalTrueBool(data.IsPaused) && optionalTrueBool(data.IsPlaying) {
		actions = append(pausedInPlayModeNextActions(), actions...)
	}
...
// single-flight gate; the caller can wait forever. Why these two recovery steps and no
...
func pausedInPlayModeNextActions() []string {
	return []string{
		...
		"Or release the pause in the Editor (Edit > Play Mode > Pause). Frames resume, so the running command finishes and returns its result.",
	}
}
