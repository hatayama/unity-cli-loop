...
const (
	...
	// Diagnoses a completed --trigger that Unity rejected (Success:false) or that failed to
	// dispatch. Why before the IsPaused / auto-cleared / non-firing branches: those assume the
	// trigger fired and the marker still missed, which is the opposite of this case and is what
	// sent agents into execute-dynamic-code loops on a paused-PlayMode input rejection.
	pausePointHintTriggerRejected = "The trigger command ran but was rejected. Read Details.TriggerResult.Response.Message for the reason (for example, input commands are rejected while PlayMode is paused by an earlier pause-point hit). Resume PlayMode (clear-pause-point / control-play-mode --action Play), re-enable the marker, and retry."
