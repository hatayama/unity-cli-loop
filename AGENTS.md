...
When changing Go CLI source files under `Packages/src/Cli~/Core~` or `Packages/src/Cli~/Shared~`, run `scripts/check-go-cli.sh` before manually rebuilding checked-in binaries.
