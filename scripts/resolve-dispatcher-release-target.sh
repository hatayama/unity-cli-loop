...
VERSION=$(jq -r '.dispatcherVersion' cli/dispatcher/dispatcher-contract.json)
if [ -z "$VERSION" ] || [ "$VERSION" = "null" ]; then
  echo "Could not resolve dispatcherVersion from cli/dispatcher/dispatcher-contract.json." >&2
  exit 1
fi
