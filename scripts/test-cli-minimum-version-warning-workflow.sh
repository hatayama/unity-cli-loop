...
assert_contains "$WARNING_WORKFLOW" "          go-version-file: cli/.go-version"
...
assert_contains "$BUILD_WORKFLOW" "      - name: Check CLI minimum version bump"
assert_contains "$BUILD_WORKFLOW" "          PR_NUMBER: \${{ github.event.pull_request.number }}"
assert_contains "$BUILD_WORKFLOW" "          CLI_MINIMUM_VERSION_FAIL_ON_WARNING: true"
assert_contains "$BUILD_WORKFLOW" "        run: scripts/check-cli-minimum-version-warning.sh"
