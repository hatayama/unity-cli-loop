...
#     [--pause-every N]     run a PlayMode pause-point cycle every N iterations (default: 5, 0 = never)
...
# Opens the soak scene, creating and saving it first if the asset is missing.
# Refuses to proceed over unsaved user scene changes (returns DIRTY_SCENE).
...
UnityEngine.GameObject go = new UnityEngine.GameObject("SoakTicker");
go.AddComponent<UloopSoakTicker>();
