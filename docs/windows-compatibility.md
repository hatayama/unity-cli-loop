...
this document is about. **Its Windows behaviour has not yet been verified on a real machine** — the
feature was developed and verified on macOS only. Treat the notes below as the design intent to
check against, not as an observed result.
...
- **Separators in response files.** Bee writes forward slashes inside `.rsp` files on both
  ...
  the OS is rebuilt with `filepath.Join` / `filepath.FromSlash`.
- **Diagnostic paths.** The `File` field of a diagnostic is whatever `csc` prints, which follows the
  spelling of the source path in the response file.
...
4. Introduce a deliberate compile error and check the separator in the diagnostic's `File` field.

If a `.rsp` turns out to use `\` and the `-r:` reference rewrite stops matching, normalize the
comparison with `filepath.ToSlash` on both sides and add a regression test that feeds a
backslash-spelled reference — a macOS-only test cannot catch it.
