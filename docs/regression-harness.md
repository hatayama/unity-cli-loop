...
| Manual hot-reload demo (no driver script): a uGUI `Image` spins around the screen center at a literal degrees-per-second inside `Update`. Open the scene, enter PlayMode, edit the literal (or flip its sign) in `SpinningImageRotator.cs`, run `uloop hot-reload --files` on it, and watch the spin change without a domain reload | `Assets/RegressionHarness/HotReloadSpinningImage/` | — |
