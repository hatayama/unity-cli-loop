...
| Manual hot-reload demo (no driver script): a uGUI `Image` orbits the screen center; the orbit radius and degrees-per-second are literals inside `Update`. Open the scene, enter PlayMode, edit either literal in `SpinningImageRotator.cs`, run `uloop hot-reload --files` on it, and watch the orbit change without a domain reload | `Assets/RegressionHarness/HotReloadSpinningImage/` | — |
| Manual EditMode hot-reload demo (no driver script): an `EditorWindow` (menu `Window/uloop/HotReload EditMode Demo`) orbits a square around the window center while the Editor is not playing; radius, size, color, and speed are literals inside `DrawOrbit` / `Advance`. Edit one, run `uloop hot-reload --files` on the file, and the window changes without PlayMode or a domain reload | `Assets/RegressionHarness/HotReloadEditorWindow/` | — |
