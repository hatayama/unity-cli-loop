...
| Annotated screenshot element dropouts and coordinate mismatch: S1/S2 pin by-design exclusions (no GraphicRaycaster / unresolved world-space camera), S3 asserts a center-blocked but still-clickable button stays annotated and that SimX/SimY click the button, S4 records SiblingIndex label-order vs draw order as a documented limitation (INFO only), S5 pins ElementsOnly SimY/bounds against a full rendering capture | `Assets/RegressionHarness/AnnotatedScreenshotMismatch/` | `scripts/regression-harness-annotated-screenshot-mismatch.sh` |
...
| Hot reload of an added field plus an added method in the same file: existing `Update` starts calling the new method, patched `WriteAdded` stores `10`, re-apply keeps `10`, `--status` lists Kind `Added`, and `--revert-all` restores the baseline marker. A second pass adds a method that reads a compiled private field and PlayMode logs that value. A newly added Unity message is carried by a proxy component during Play Mode, so this harness keeps using the existing `Update` to pin the added-member path on its own | `Assets/RegressionHarness/HotReloadAddedMember/` | `scripts/regression-harness-hot-reload-added-member.sh` |
| Wired added-field values across the Play Mode scene reload with Domain Reload disabled (the script exits 1 when Domain Reload is on): an added `Transform` field wired to the scene's `Target` in Edit Mode reads `Target` in the rebuilt host's `Awake` (while the scene loads) and `Update`, `--status` reports `RestoredWiredValueCount >= 1`, and the Edit Mode host rebuilt after Stop reads it too. After `--revert-all` and a re-apply without wiring, `Update` logs `update-target=none` | `Assets/RegressionHarness/HotReloadWiredValuePersistence/` | `scripts/regression-harness-hot-reload-wired-value-persistence.sh` |
| Hot reload of a return-type change: same-file caller is patched and PlayMode logs the new value; a compiled caller in another file reports the changed method and its edited same-file caller as `Skipped`. `--revert-all` restores the baseline marker | `Assets/RegressionHarness/HotReloadSignatureChange/` | `scripts/regression-harness-hot-reload-signature-change.sh` |
| Pause-point capture on a hot-reload patched method: compile-time (A) and post-`hot-reload` re-arm (B) on the same `body.Move` line must both capture post-update locals (`step>0`, `motion` non-zero) and the field after the gravity line. A capture that lands at the spurious earlier shim sequence point instead shows `step=0`, `motion=(0,0,0)`, `verticalVelocity=-1` | `Assets/RegressionHarness/PatchedMethodPausePointCapture/` | `scripts/regression-harness-patched-method-pause-point-capture.sh` |
| External `.cs` and open-scene changes made while the Editor is unfocused are imported, compiled, and reloaded on focus return without a modal dialog (the Auto Refresh hold removed for #2575 used to leave the `.cs` unimported). A second pass then edits the open scene while focused and checks that `uloop compile` imports and reloads it without the native dialog | `Assets/RegressionHarness/FocusReturnAutoRefresh/` | `scripts/regression-harness-focus-return-auto-refresh.sh` |
| Play Mode with an active hot-reload patch survives `uloop focus-window`. Native Auto Refresh must not import the edited `.cs` and stop Play (`ScriptCompilationDuringPlay=2`). After Stop, domain reload (when enabled) or `uloop compile` clears the ledger and `AutoRefreshHeld` | `Assets/RegressionHarness/HotReload/` (reused) | `scripts/regression-harness-hot-reload-focus-return.sh` |
| Manual hot-reload demo (no driver script): a uGUI `Image` orbits the screen center; the orbit radius and degrees-per-second are literals inside `Update`. Open the scene, enter PlayMode, edit either literal in `SpinningImageRotator.cs`, run `uloop hot-reload --files Assets/RegressionHarness/HotReloadSpinningImage/SpinningImageRotator.cs`, and watch the orbit change without a domain reload | `Assets/RegressionHarness/HotReloadSpinningImage/` | — |
| Manual EditMode hot-reload demo (no driver script): an `EditorWindow` (menu `Window/uloop/HotReload EditMode Demo`) orbits a square around the window center while the Editor is not playing; radius, size, color, and speed are literals inside `DrawOrbit` / `Advance`. Edit one, run `uloop hot-reload --files Assets/RegressionHarness/HotReloadEditorWindow/Editor/HotReloadEditorWindowDemo.cs`, and the window changes without PlayMode or a domain reload | `Assets/RegressionHarness/HotReloadEditorWindow/` | — |
