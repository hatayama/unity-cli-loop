...
reload. Reload in two steps: first without the change, which introduces the new type, then the
change together with an edit of the new type (a method body change is enough).
