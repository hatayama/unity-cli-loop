...
`--timeout-seconds` on enable starts the marker lifetime clock at enable time, not when you later run `await-pause-point`. You do not need to inflate this value to cover a slow multi-step CLI sequence between `enable-pause-point` and `await-pause-point` (for example seeding state with `execute-dynamic-code` first): `await-pause-point` extends the marker's remaining lifetime to at least its own `--timeout-seconds` right before it starts waiting, so earlier setup steps cannot eat into the window it observes.
...
- When every reachable line around the state change you want runs unconditionally every frame, with no existing `if` to hang the pause point on, wrap a temporary `if (<postcondition>) { Debug.Assert(<postcondition>); }` around the moment you want to observe and target the pause point at the `Debug.Assert` line. The `if` is what creates the conditional line the pause point needs; the `Debug.Assert` is a natural, low-friction choice to put inside it because this codebase already uses `Debug.Assert` to state postconditions, so the temporary line reads as intent ("this must hold here") rather than as an arbitrary probe, and it compiles away without changing behavior once removed.
