...
Run this in the background only because the next step needs to run a `simulate-*` command while this call is still blocking. When nothing has to run in parallel (the game reaches the line on its own), run the same call in the foreground instead: it prints the hit result with `CapturedVariables` directly to stdout when it unblocks. When you must guarantee the marker is armed before sending input, split the steps: run `enable-pause-point` without `--await` in the foreground (its response returning is the arm confirmation), then start `uloop await-pause-point --id <id>` in the background, then send the input. Do not approximate arm-waiting with a fixed sleep after a backgrounded enable.
