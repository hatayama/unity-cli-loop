...
6. A `single-shot` marker (the default) disarms itself after the hit, so no clear call is required before moving on. For `continuous`/`trace` markers, or to remove the underlying code patch before the method runs untouched again, clear it with `uloop clear-pause-point --id "Assets/Scripts/Enemy.cs:42"` (or `--all` to clear every active marker at once, for example when resetting between E2E scenarios) or stop PlayMode.
