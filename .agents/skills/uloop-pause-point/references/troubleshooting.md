...
`enable-pause-point` works on hot-reload patched methods: the marker resolves against the patched body, and `RetargetedToHotReloadPatch: true` in the response confirms it is armed on the edited code — that flag is not a problem; it means the marker follows the patched body and keeps firing at the edited line. Methods the reload did not patch are the opposite case: `--line` on them resolves against the last compiled source, not the edited file, so line drift from the edit can silently arm a different method. Pass `--method` with the simple method name or `Type.Method` to keep `--line` inside that method and fail instead of arming a neighbor. A "Candidate:" line number is a last-compiled-source line for orientation only; do not pass it as --line, which is read against the edited file. Retry with --method <Type.Method> and the edited --line; the line is then matched by its text inside that method. The response carries a Warning when this applies — check `ResolvedMethod` and `ResolvedLineText` before trusting the marker, or run `uloop compile` and re-enable. When the statement text at the resolved line is identical in the edited file, the Warning says so and no manual comparison is needed. A line made only of braces, parentheses or semicolons does not count as identical, because it matches almost anywhere. When the resolved line number falls inside a hot-reload patched method's edited body, the Warning says the file drifted and names that patched method instead of comparing text. When the compiled statement at the resolved line differs from the edited file, the response also includes a compiled-line drift Warning and RecommendedNextAction. `CapturedVariables` never includes fields added by hot reload (their values live in a side table); enable-pause-point warns when the resolved type has any. While paused, read one from `execute-dynamic-code` with `HotReloadAddedFieldWiring.TryReadInstanceField` (`TryReadStaticField` for a static field), as `references/added-field-wiring.md` in the uloop-hot-reload skill shows; naming the field as a member fails with CS1061.
...
A method hot reload *added* (an `Added` row) cannot hold a pause point: it exists only in the hot reload shim and has no line in the compiled line map. A line inside it is refused with `PAUSE_POINT_RESOLVE_FAILED` and a message naming that added method; it is never armed on another method instead. Run `uloop compile`, then enable the pause point on that line again. The one exception is a last-compiled-source line of a compiled method that the added method's edited lines happen to cover: pass `--method` with that compiled method's name, and the pause point arms there (`LineBasis: LastCompiledSource`) only when the compiled method's span holds `--line` and the resolved line stays inside it. `--method` naming the added method, or a method whose compiled span does not hold the line, is still refused. A bare method name that also names the added method (an added `Owner.Step` over a compiled `Helper.Step`) is refused as ambiguous; pass it as `Type.Method` (`--method Helper.Step`) to arm the compiled method.

`PAUSE_POINT_PATCHED_BY_HOT_RELOAD` is returned only when the requested line cannot be mapped onto the patched body. When the compiled line range of the patched method is known, the line lies outside every patched body but maps into that range from above or below it, and the next action leads with `--method <Type.Method> --line N` for a line inside an unpatched method; the line is then matched by its text inside that method's compiled span or on its declaration lines. The failure message also reports the range, so you can see how far the edited file's line numbers have shifted from the compiled source. A blank or comment line maps the same way; pick a statement line, or a line inside the edited body to pause in the patched method. `uloop compile` realigns line numbers but ends the active patches; `uloop hot-reload --revert-all` also ends them. When the range is unknown, the older guidance stays: pick a line inside the edited method body, run `uloop hot-reload --revert-all`, or run `uloop compile`, then retry.

`PAUSE_POINT_PATCHED_SOURCE_CHANGED` is returned when `--line` falls inside a hot-reload patched method but the file changed on disk after that patch was applied — the last reload of the file did not apply (for example a compile error), or the file was edited without reloading. The running patch still follows the line numbers of the source it was compiled from, so a marker placed now would stop at a different statement than the one shown. Run `uloop hot-reload` on the file again (fix any compile error first) or revert the edit, then retry. Lines outside every patched method are not affected.
