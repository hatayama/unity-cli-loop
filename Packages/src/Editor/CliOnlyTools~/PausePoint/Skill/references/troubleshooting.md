...
`PAUSE_POINT_PATCHED_BY_HOT_RELOAD` is returned when the edited line resolves to a statement of a hot-reload patched method's compiled body, which no longer runs. The message names the method and its edited body range; retry with `--line` inside that range. When the range is unknown, pick a line inside the edited method body or run `uloop compile`. A patched method whose patch has no debug symbols is refused the same way; run `uloop compile` and re-enable.
