...
- The physics-callback warning matches the target method name only. A regular method that is *called from* a physics callback (for example a helper invoked inside `OnCollisionEnter2D`) gets no warning but can still miss existing GameObjects for the same reason — treat it as the same limitation.
