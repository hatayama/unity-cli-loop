...
The scene contains a deterministic 4x4 set of `BoxCollider` tiles and a center `GraphicRaycaster` UI blocker. Verify relative behavior rather than fixed pixel coordinates: translucent cells should appear only where raycast samples remain reachable, center tiles should shrink around the blocker, frames should wrap those reachable cells, and `SimX/SimY` should stay inside each frame.
