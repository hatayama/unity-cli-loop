...
[English](whats-new-v3.md) | 日本語

V3では、npmで配布していたCLIをネイティブのGoバイナリに置き換え、通信方式もTCPポート管理からOSネイティブのIPCに変えました。AIエージェントからUnityを操作するのに、Node.jsのセットアップもポートの管理も不要になります。接続の信頼性も改善しました。Unityがバックグラウンドにあっても、複数のUnity Editorを並列で動かしていても、接続は安定して保たれます。

CLIのバージョン管理も不要になりました。V2ではパッケージを更新するたびに同じバージョンのCLIを手で入れ直す必要がありましたが、V3では一度インストールすれば以後の更新は自動で行われます。さらに、プロジェクトごとに必要なCLIバージョンを自動で使い分けるため、パッケージバージョンの異なる複数のプロジェクトを並行して触っても、CLIを切り替える作業は一切ありません。

機能面の目玉は `hot-reload` と `pause-point` です。`hot-reload` は、編集したメソッド本体を、再コンパイルもDomain Reloadも挟まずに実行中のEditorへ即時反映します。`pause-point` は、ソースを編集することも再コンパイルすることもなく、任意の行でPlayModeを停止して、その瞬間の変数を読み取れます。特定の瞬間のスクリーンショットを撮ったり、特定の瞬間のHierarchyを調査したりするのにも便利です。

MCP対応は削除され、CLIとSkillsが唯一の連携方法になりました。それぞれの詳細は、以下の各セクションを参照してください。
...
### `hot-reload` — メソッド本体の編集を再コンパイルなしで即時反映

`hot-reload` は、編集した `.cs` ファイルのメソッド本体を、再コンパイルもDomain Reloadも挟まずに実行中のEditor（EditMode / PlayMode）へ直接反映します。属性やソースマーカーは不要で、private / internalメンバーへのアクセス、staticメソッド、asyncメソッド、イテレータも動作します。PlayModeを止めずにゲームロジックを修正して、その場で挙動を確かめられます。

新しいメソッドやフィールドの追加も適用できます（追加したメンバーは、同じファイル内の編集済みコードからのみ参照できます）。新しい型の追加や、他ファイルから参照されるメンバーの追加には、従来どおり `uloop compile` が必要です。反映できないメソッドはメソッド単位で `Skipped` / `Failed` として報告され、1つのメソッドが失敗しても残りの適用は止まりません。

```bash
# 指定したファイルのメソッド本体を実行中のEditorへ適用
uloop hot-reload --files Assets/Scripts/Enemy.cs

# 引数なしで、最後のコンパイル以降に変更された.csを自動選択して適用
uloop hot-reload

# 適用中の変更一覧の確認と、適用した変更すべての解除
uloop hot-reload --status
uloop hot-reload --revert-all
```

hot reloadは探索フェーズ、`uloop compile` は着地フェーズとして使い分けてください。コンパイルやDomain Reloadが起きると、hot reloadで反映した変更はすべて解除されます（ソースの編集自体は残り、コンパイルで通常どおり反映されます）。
