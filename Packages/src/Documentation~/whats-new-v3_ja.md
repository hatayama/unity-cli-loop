...
機能面の目玉は `hot-reload` と `pause-point` です。`hot-reload` は、編集したメソッド本体をDomain Reloadを挟まずに実行中のEditorへ即時反映します。`pause-point` は、ソースを編集することも再コンパイルすることもなく、任意の行でPlayModeを停止して、その瞬間の変数を読み取れます。特定の瞬間のスクリーンショットを撮ったり、特定の瞬間のHierarchyを調査したりするのにも便利です。
...
### `hot-reload` — メソッド本体の編集をDomain Reloadなしで即時反映

`hot-reload` は、編集した `.cs` ファイルのメソッド本体を、Domain Reloadを挟まずに実行中のEditor（EditMode / PlayMode）へ直接反映します。属性やソースマーカーは不要で、private / internalメンバーへのアクセス、staticメソッド、asyncメソッド、イテレータも動作します。PlayModeを止めずにゲームロジックを修正して、その場で挙動を確かめられます。

新しいメソッドやフィールドの追加も適用できます（追加したメンバーは、同じファイル内の編集済みコードからのみ参照できます）。新しい型の追加や、他ファイルから参照されるメンバーの追加には、従来どおり `uloop compile` が必要です。パッチできないメソッドはメソッド単位で `Skipped` / `Failed` として報告され、1つのメソッドが失敗しても残りの適用は止まりません。
...
# 適用中の変更一覧の確認と、全パッチの解除
uloop hot-reload --status
uloop hot-reload --revert-all
```

hot reloadは探索フェーズ、`uloop compile` は着地フェーズとして使い分けてください。コンパイルやDomain Reloadが起きると、パッチはすべて解除されます。
