...
V3では、AIエージェントからUnityを操作するのに、Node.jsのセットアップもポートの管理も不要になりました。接続の安定性も向上しました。
...
- **インストーラの真正性検証** — リリース成果物にはsigstoreのattestationが付いています。インストーラは、ダウンロードしたアーカイブをリポジトリに固定されたダイジェストと照合してから実行します。
