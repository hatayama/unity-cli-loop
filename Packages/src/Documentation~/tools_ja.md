...
### 4. hot-reload - メソッド本体の変更をDomain Reloadなしで即時適用
編集した `.cs` ファイルのメソッド本体を、Domain Reloadを挟まずに実行中のEditor（EditMode / PlayMode）へ直接反映します。属性やソースマーカーは不要で、private / internalメンバーへのアクセス、staticメソッド、asyncメソッド、イテレータも動作します。PlayModeを止めずにゲームロジックを修正して、その場で挙動を確かめられます。

新しいメソッドやフィールドの追加も適用できます（追加したメンバーは、同じファイル内の編集済みコードからのみ参照できます）。新しい型の追加や、他ファイルから参照されるメンバーの追加には `compile` が必要です。パッチできないメソッドはメソッド単位で `Skipped` / `Failed` として報告され、1つのメソッドが失敗しても残りの適用は止まりません。

> [!NOTE]
> hot reloadは探索フェーズ、`compile` は着地フェーズとして使い分けてください。コンパイルやDomain Reloadが起きると、パッチはすべて解除されます。適用中の変更は `--status` で確認できます。
