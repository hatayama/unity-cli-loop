...
新しいメソッドやフィールドの追加も適用できます（追加したメンバーは、同じファイル内の編集済みコードからのみ参照できます）。新しい型の追加や、他ファイルから参照されるメンバーの追加には `compile` が必要です。反映できないメソッドはメソッド単位で `Skipped` / `Failed` として報告され、1つのメソッドが失敗しても残りの適用は止まりません。

事前の仕込みが一切要らないのも特徴です。既存のホットリロード手法には、対象メソッドにあらかじめ属性を付けてコンパイルしておく必要があるものや、対象にできるメソッドの種類が限定されるものがあります。uloopのhot reloadはこうした事前準備や制限なしで、すでに実行中のPlayModeの任意のメソッドに後から適用できます。メソッド本体の編集にとどまらず、ローカル変数の型や、メソッドの戻り値・引数といったシグネチャの変更も反映できます（旧シグネチャを呼び出しているメソッドが同じリロードでまとめて更新される場合）。

> [!NOTE]
> hot reloadは探索フェーズ、`compile` は着地フェーズとして使い分けてください。コンパイルやDomain Reloadが起きると、hot reloadで反映した変更はすべて解除されます（ソースの編集自体は残り、コンパイルで通常どおり反映されます）。適用中の変更は `--status` で確認できます。

```text
→ hot-reload (Files: ["Assets/Scripts/Enemy.cs"])
→ PlayModeを止めずに、修正後の挙動をその場で確認
→ 引数なしのhot-reloadで、最後のコンパイル以降に変更されたファイルを自動選択して適用
→ 構造的な変更がまとまったらcompileで着地
```

### 5. pause-point - コードを書き換えずに任意の行で止めて変数を見る
...
### 7. find-game-objects - シーン内オブジェクト検索
