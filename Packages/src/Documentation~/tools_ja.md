...
事前の仕込みが一切要らないのも特徴です。属性ベースのホットリロードでは、対象メソッドにあらかじめ属性を付けてコンパイルしておく必要があり、対象メソッドの種類が限定されたり、差し替えのためにメソッド本体を専用ヘルパー経由へ書き換える必要があったりします。uloopのhot reloadはこうした事前準備なしで、すでに実行中のPlayModeの任意のメソッドに後から適用できます。
