...
事前の仕込みが一切要らないのも特徴です。既存のホットリロード手法には、対象メソッドにあらかじめ属性を付けてコンパイルしておく必要があるもの、対象にできるメソッドの種類が限定されるもの、差し替えのためにメソッド本体を専用ヘルパー経由へ書き換える必要があるものがあります。uloopのhot reloadはこうした事前準備や制限なしで、すでに実行中のPlayModeの任意のメソッドに後から適用できます。
