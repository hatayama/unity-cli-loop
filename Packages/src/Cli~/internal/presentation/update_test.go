...
import (
	...
	clicontract "github.com/hatayama/unity-cli-loop/Packages/src/Cli"
	"github.com/hatayama/unity-cli-loop/Packages/src/Cli/internal/adapters/installer"
)
...
func TestUpdateCommandForDarwinUsesDirectInstaller(t *testing.T) {
	...
	expectedScriptURL := installer.ScriptURL(clicontract.Current.CliVersion, installer.PosixScriptName)
	expectedReleaseTag := installer.UpdateSelectorForVersion(clicontract.Current.CliVersion)
...
func TestUpdateCommandForWindowsUsesPowerShellInstaller(t *testing.T) {
	...
	joinedArgs := strings.Join(args, " ")
	expectedScriptURL := installer.ScriptURL(clicontract.Current.CliVersion, installer.WindowsScriptName)
	expectedReleaseTag := installer.UpdateSelectorForVersion(clicontract.Current.CliVersion)
	if !strings.Contains(joinedArgs, expectedScriptURL) {
		t.Fatalf("installer URL missing: %s", joinedArgs)
	}
