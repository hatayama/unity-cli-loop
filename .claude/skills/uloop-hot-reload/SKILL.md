...
- `AddedFields` (array): source-level names ("Type.field") of fields this reload added; their values live outside the compiled type until 'uloop compile'. Every run that adds fields also carries a warning that the values live outside the compiled assembly and last only until the next 'uloop compile' or domain reload.
