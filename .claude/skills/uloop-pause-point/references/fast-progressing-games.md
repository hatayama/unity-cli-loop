...
One case skips the resume on purpose: if the marker had **already** hit before the wait started (a single-shot marker, or any marker whose mode does not wait for a new hit), the wait settles on that recorded hit instantly, so resuming would restart the game and then report an old hit. The wait returns the recorded hit and reports `ResumePlayResult.Skipped` (with `Resumed: false` and no `Error`); a `--trigger` given alongside is not dispatched and says so in `TriggerResult.Error`. Unity stays paused by that hit — clear or re-enable the marker and wait again to capture a new one. An already-hit `continuous`/`trace` marker is unaffected: its wait needs a later hit, so it still resumes.
