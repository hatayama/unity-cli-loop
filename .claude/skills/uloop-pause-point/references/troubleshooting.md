...
A method hot reload *added* (an `Added` row) cannot hold a pause point: it exists only in the hot reload shim and has no line in the compiled line map. A line inside it is refused with `PAUSE_POINT_RESOLVE_FAILED` and a message naming that added method, with or without `--method`; it is never armed on another method instead. Run `uloop compile`, then enable the pause point on that line again.
