...
- `--snapshot-timing post-line` captures **after** the resolved line's own statement finished, just before control leaves it. Use it to read the value an assignment or call on that line produced without arming the next line — which may be a join point that an early `return` or the `else` branch also reaches, so a marker there fires for executions that never ran the requested line. Post-line semantics per statement shape:
  - `if (cond)` / `while (cond)`: the capture runs after the condition was evaluated, whichever way the branch goes; it never runs inside the body. A `for` header resolves to its initializer, so the capture runs after `i = 0` and before the first condition check; loop iterations are not captured.
...
`await-pause-point`'s hit response also carries a top-level `Warning` (omitted when empty): it flags multiple hits, multiple matching logs, or truncated matching logs, so you can tell a single clean hit apart from evidence that needs closer inspection. The hit response has no `Warnings` array; those hit-time topics are the `Warning` string only. Enable-time patch diagnostics (for example physics-callback cached dispatch) are not in that hit-time `Warning`; on `enable-pause-point --await` they appear as `EnableTimeWarning` / `EnableTimeWarnings` instead. The enable response itself (before await) lists those same topics as `Warnings`. `MatchingLogs` (log entries whose text contains the marker id) is still embedded, but source-derived ids rarely appear in log text, so treat `CapturedVariables` as the primary variable evidence.
