...
`--timeout-seconds` on enable starts the marker lifetime clock at enable time, not when you later run `await-pause-point`. This value only needs to cover the setup you do between `enable-pause-point` and calling `await-pause-point` (for example seeding state with `execute-dynamic-code` first) — the marker still expires if setup runs longer than that, since a marker cannot be resurrected once expired. It no longer needs to also cover the wait itself: `await-pause-point` extends the marker's remaining lifetime to at least its own `--timeout-seconds` right before it starts waiting, so you do not need to add the wait's duration on top when sizing `enable-pause-point --timeout-seconds`.
...
- Reaching a value through nested fields off `this` (or off any captured object) bottoms out at type-name-only text past `MaxCollectionPreviewDepth` (2 levels), and this depth budget is independent per captured variable — going deep through `this` does not borrow depth from other variables in the same snapshot, nor vice versa. If the value you actually need is more than two levels deep, pick a pause point line where that value is itself a direct local or parameter instead of a nested field, so it gets its own top-level captured entry with its own full depth budget.
...
- A captured `UnityEngine.Object` value's `Value` string only ever carries identifying information (its `name` and similar identifiers), never its custom fields — a `MonoBehaviour`'s own `[SerializeField]` values do not appear in that entry. When you need a custom field's value, either expose it through the component's own `.name`/`ToString()` so it appears in the identifier, or read it directly while still paused with `execute-dynamic-code` off the live instance (via `UnityObjectPath`/`UnityObjectInstanceId`, or `UloopPausePoint.TryGetCapturedValue("this")` for the paused instance itself).
...
For a self-progressing game (a board that keeps advancing, an opponent that keeps moving), arranging a specific scenario through real input alone is a race against the game clock. While paused on a hit, use `TryGetCapturedValue("this")` to get the live instance and call its production methods directly to build up the board/state you need, reserving actual simulated key/mouse input for only the one move you are actually verifying. This keeps the setup deterministic while still exercising the real input path for the observed action itself.
...
- When every reachable line around the state change you want runs unconditionally every frame, with no existing `if` to hang the pause point on, wrap a temporary `if (<event condition>) { <mutation>; Debug.Assert(<postcondition of the mutation>); }` around the moment you want to observe and target the pause point at the `Debug.Assert` line. The `if` is what creates the conditional line the pause point needs, guarding on the event you actually care about rather than repeating the postcondition it produces; the `Debug.Assert` line right after the mutation is a natural, low-friction place to put the pause point because this codebase already uses `Debug.Assert` to state postconditions, so the temporary line reads as intent ("this must hold here") rather than as an arbitrary probe.
