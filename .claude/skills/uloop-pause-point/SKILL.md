...
- `--captured-variables` defaults to `full` (every local, parameter, and InstanceField). When that dump is noisy, pass `--captured-variable-names name1,name2` to keep only those names; it composes with `full` or `names`.
