...
3. Drive the scene with other uloop commands (`simulate-keyboard`, `simulate-mouse-input`, `replay-input`, ...). Do **not** run `uloop compile` mid-recording: it auto-stops and finalizes the file. Exiting Play Mode also auto-stops a Game View recording; a `--window-name` recording keeps running when Play Mode stops.
